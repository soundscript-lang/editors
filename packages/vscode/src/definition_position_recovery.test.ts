...
import { mapSourcePositionToProjected } from './projection_mapping';
...
test('recoverProjectedIdentifierOffset restores same-file definitions inside macro replacements', () => {
  const sourceOffset = SNAPSHOT.originalText.indexOf('safeDivide(a, b)') + 1;
  const rawProjected = mapSourcePositionToProjected(SNAPSHOT, sourceOffset);
  assert.equal(rawProjected.insideReplacement, true);

  const lookupFileName = '/workspace/src/index.ts';
  const service = createLanguageService(lookupFileName, SNAPSHOT.projectedText);

  const rawDefinitions = service.getDefinitionAtPosition(
    lookupFileName,
    rawProjected.position,
  ) ?? [];
  assert.notEqual(rawDefinitions[0]?.textSpan.start, 184);

  const recoveredOffset = recoverProjectedIdentifierOffset(
    SNAPSHOT,
    sourceOffset,
    rawProjected.position,
  );
  assert.notEqual(recoveredOffset, undefined);

  const recoveredDefinitions = service.getDefinitionAtPosition(
    lookupFileName,
    recoveredOffset!,
  ) ?? [];
  assert.equal(recoveredDefinitions[0]?.textSpan.start, 184);
});
